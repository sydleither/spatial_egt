...
def significance_table(df, group_type, groups, sim_types):
...
def main(exp_dir, analysis_type, fixed_type, fixed_var):
    sim_types = ["null", "continuous"]
    df = read_all(exp_dir)
    df_fixed = df.loc[df[fixed_type] == fixed_var]

    groups = sorted(df_fixed[analysis_type].unique())
    significance_table(df_fixed, analysis_type, groups, sim_types)
...
if __name__ == "__main__":
    if len(sys.argv) == 5:
        ...
        print("Please provide an experiment directory, type of analysis, "+ 
              "the fixed variable type, and the fixed variable. Examples:")
