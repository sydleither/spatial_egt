...
def get_times_to_progression(df):
    ...
    for threshold in sorted(df["threshold"].unique()):
        ...
        for experiment in df["experiment"].unique():
            ...
            for rep in df_thr["rep"].unique():
                df_rep = df_exp.loc[df_exp["rep"] == rep]
                progression_amt = 1.2 * df_rep.loc[df_rep["time"] == 0]["adaptive_total"].tolist()[0]
                progressed = df_rep.loc[df_rep["adaptive_total"] >= progression_amt]["time"].tolist()
                if len(progressed) == 0:
...
def threshold_plot(df, exp_dir, dimension):
    ...
    df_pt = get_times_to_progression(df_d)
    ymin = df_pt["progression_time"].min()
    ymax = df_pt["progression_time"].max()
    colors = ["sienna", "green", "steelblue"]
    thresholds = sorted(df["threshold"].unique())
    for i,experiment in enumerate(experiments):
        plot_line(axis[i], df_pt.loc[df_pt["experiment"] == experiment], "threshold", "progression_time", colors[i], None)
