...
def experiment_config(exp_dir, config_name, runNull, runAdaptive, runContinuous, writePopFrequency,
                      writeModelFrequency, numTicks, radius, deathRate, drugGrowthReduction, numCells, 
...
def sample_games(exp_dir, name, a, b, c, d, runNull=1, runAdaptive=0, runContinuous=0, 
                 writePopFrequency=1, writeModelFrequency=0, radius=1, turnover=0.009,
                 drug_reduction=0.5, init_cells=4375, prop_res=0.01, adaptiveTreatmentThreshold=0.5,
...
if __name__ == "__main__":
    ...
    if experiment_name == "sample":
        ...
        for i in range(N):
            sample = samples[i]
            s, a = sample_games(writePopFrequency=0, writeModelFrequency=250, runtime=250,
